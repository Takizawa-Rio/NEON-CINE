...
class MovieViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // Tập hợp mã vé / barcode đã bị xóa để lọc tức thì trên giao diện
    private val _deletedTicketBarcodes = MutableStateFlow<Set<String>>(
        prefs.getStringSet("deleted_ticket_barcodes", emptySet()) ?: emptySet()
    )
    ...
    val tickets: StateFlow<List<Ticket>> = combine(
        ...
        _deletedTicketBarcodes
    ) { dbTickets, localTickets, deletedSet ->
        val combined = (localTickets + dbTickets).filter { t ->
            !deletedSet.contains(t.barcode) &&
            !deletedSet.contains(t.bookingCode) &&
            (t.id <= 0 || !deletedSet.contains(t.id.toString())) &&
            !deletedSet.contains("${t.movieTitle}_${t.dateTime}_${t.seats}")
        }.distinctBy { 
            if (it.bookingCode.isNotBlank()) it.bookingCode else if (it.barcode.isNotBlank()) it.barcode else "${it.movieTitle}_${it.dateTime}_${it.seats}"
    ...
    val myTickets: StateFlow<List<Ticket>> = combine(
        ...
        val unifiedList = (localCreated + allTickets).distinctBy {
        ...
        unifiedList.filter { t ->
            val tEmail = t.userEmail.trim().lowercase()
            val tName = t.userName.trim().lowercase().replace(" ", "")

            // 1. Khớp email hoặc tên người dùng của tài khoản đang đăng nhập
            val isEmailMatch = emailLower.isNotEmpty() && (
                (tEmail.isNotEmpty() && (tEmail == emailLower || (emailPrefix.isNotEmpty() && tEmail.contains(emailPrefix)))) ||
                (tEmail.isNotEmpty() && tEmail == nameClean)
            )
            val isNameMatch = nameClean.isNotEmpty() && (
                (tName.isNotEmpty() && (tName == nameClean || tName.contains(nameClean) || nameClean.contains(tName))) ||
                (tEmail.isNotEmpty() && (tEmail == nameClean || tEmail.contains(nameClean) || nameClean.contains(tEmail)))
            )
            val isLocalForUser = localCreated.any {
                (it.barcode == t.barcode || (it.bookingCode.isNotEmpty() && it.bookingCode == t.bookingCode)) &&
                (it.userEmail.isBlank() || it.userEmail.equals(emailLower, ignoreCase = true) || it.userName.equals(name, ignoreCase = true))
            }
            
            isEmailMatch || isNameMatch || isLocalForUser
        }
    }.stateIn(
        scope = viewModelScope,
        started = SharingStarted.Eagerly,
    ...
    // Danh sách ngày chiếu động từ Supabase 100% (Chỉ lấy từ hôm nay trở đi, không dùng ngày giả lập)
    ...
    // Tất cả các ngày có lịch chiếu trong hệ thống Supabase (dùng cho tab Lịch chiếu chung)
    val allAvailableShowtimeDates: StateFlow<List<String>> = _showtimes.map { list ->
        list.filter { it.date.isNotBlank() && isDateTodayOrFuture(it.date) }
            .map { formatShowtimeDate(it.date) }
            .distinct()
            .sortedBy { parseDateForSort(it) }
    }.stateIn(
        scope = viewModelScope,
        started = SharingStarted.Eagerly,
        initialValue = emptyList()
    ...
    // Lấy danh sách suất chiếu cho một phim và ngày chiếu cụ thể (100% từ Supabase DB)
    ...
    // Danh sách suất chiếu theo thời gian động từ cơ sở dữ liệu Supabase 100% (không dùng khung giờ tĩnh)
    val availableTimeSlots: StateFlow<List<String>> = combine(_selectedMovie, _selectedDate, _showtimes) { movie, date, list ->
        if (movie != null && date.isNotBlank()) {
            val movieShowtimes = list.filter {
                (it.movieId == movie.id || (movie.stringId.isNotEmpty() && (it.movieStringId == movie.stringId || it.movieStringId == "FILM_${movie.id}" || it.movieStringId == "${movie.id}")) || (it.movieTitle.isNotBlank() && it.movieTitle.equals(movie.title, ignoreCase = true))) &&
                (it.date.isBlank() || it.date == date || isDateMatch(it.date, date))
            }.map { it.startTime }.filter { it.isNotBlank() }.distinct().sorted()
    ...
    init {
        try {
            ...
            // Vòng lặp cập nhật tự động tức thì (Real-time auto-sync) định kỳ mỗi 4 giây từ Supabase
            viewModelScope.launch {
                while (true) {
                    ...
                    try {
                        ...
                        android.util.Log.e("MovieViewModel", "Auto sync loop error: ${e.message}")
                    }
                }
            }
        } catch (e: Throwable) {
    ...
    fun selectTab(tab: Int) {
        ...
        // Khi người dùng vào trang Profile (tab 4), tự động đồng bộ ngay thông tin sang bảng customers & profiles trên Supabase
        if (tab == 4 && _isLoggedIn.value && _userEmail.value.isNotBlank()) {
            val currentProfile = UserProfile(
                email = _userEmail.value,
                name = _userName.value.ifBlank { _userEmail.value.substringBefore("@") },
                points = _userPoints.value,
    ...
    fun login(email: String, name: String) {
        ...
        // Đồng bộ ngay sang bảng customers & profiles trên Supabase
        val immediateProfile = UserProfile(email = email, name = defaultName, points = 150, balance = 500000)
        com.example.data.supabase.SupabaseSyncService.pushProfileToSupabase(immediateProfile) { }
        ...
        com.example.data.supabase.SupabaseSyncService.fetchProfileFromSupabase(email) { remoteProfile ->
            if (remoteProfile != null) {
                ...
                // Cập nhật lại thông tin mới nhất sang customers
                com.example.data.supabase.SupabaseSyncService.pushProfileToSupabase(remoteProfile) { }
            }
        }
    }
    ...
    fun selectMovie(movie: Movie?) {
        ...
        if (movie != null) {
            ...
            _selectedDate.value = upcomingDates.firstOrNull() ?: ""

            val movieTimes = if (_selectedDate.value.isNotBlank()) {
                _showtimes.value.filter {
                    (it.movieId == movie.id || (movie.stringId.isNotEmpty() && (it.movieStringId == movie.stringId || it.movieStringId == "FILM_${movie.id}" || it.movieStringId == "${movie.id}")) || (it.movieTitle.isNotBlank() && it.movieTitle.equals(movie.title, ignoreCase = true))) &&
                    (it.date == _selectedDate.value || isDateMatch(it.date, _selectedDate.value))
                }.map { it.startTime }.filter { it.isNotBlank() }.distinct().sorted()
            } else emptyList()

            _selectedTime.value = movieTimes.firstOrNull() ?: ""
            loadShowtimesAndBookingsFromSupabase()
    ...
    fun startBookingFlow() {
        ...
        if (_selectedTime.value.isBlank() || !availableTimeSlots.value.contains(_selectedTime.value)) {
            _selectedTime.value = availableTimeSlots.value.firstOrNull() ?: ""
        }
        loadShowtimesAndBookingsFromSupabase()
    ...
    fun selectDate(date: String) {
        ...
        val movie = _selectedMovie.value
        val slots = if (movie != null && date.isNotBlank()) {
            _showtimes.value.filter {
                ...
                (it.date == date || isDateMatch(it.date, date))
            }.map { it.startTime }.filter { it.isNotBlank() }.distinct().sorted()
        } else emptyList()
        _selectedTime.value = slots.firstOrNull() ?: ""
        loadShowtimesAndBookingsFromSupabase()
    }
    ...
    fun purchaseTicket(onSuccess: () -> Unit) {
        ...
        val matchedShowtime = _showtimes.value.find {
            ...
            (it.movieId == movie.id || (movie.stringId.isNotEmpty() && (it.movieStringId == movie.stringId || it.movieStringId == "FILM_${movie.id}" || it.movieStringId == "${movie.id}")) || (it.movieTitle.isNotBlank() && it.movieTitle.equals(movie.title, ignoreCase = true)))
        } ?: _showtimes.value.firstOrNull { it.uuid.isNotBlank() }
    ...
    fun deleteTicket(ticket: Ticket) {
        val updatedSet = _deletedTicketBarcodes.value.toMutableSet()
        if (ticket.barcode.isNotBlank()) updatedSet.add(ticket.barcode)
        if (ticket.bookingCode.isNotBlank()) updatedSet.add(ticket.bookingCode)
        if (ticket.id > 0) updatedSet.add(ticket.id.toString())
        updatedSet.add("${ticket.movieTitle}_${ticket.dateTime}_${ticket.seats}")
        _deletedTicketBarcodes.value = updatedSet
        prefs.edit().putStringSet("deleted_ticket_barcodes", updatedSet).apply()

        _localCreatedTickets.value = _localCreatedTickets.value.filter { 
            it.barcode != ticket.barcode && it.id != ticket.id && it.bookingCode != ticket.bookingCode 
        }
        viewModelScope.launch {
    ...
    fun clearAllTickets() {
        val allCurrent = myTickets.value
        val updatedSet = _deletedTicketBarcodes.value.toMutableSet()
        allCurrent.forEach { ticket ->
            if (ticket.barcode.isNotBlank()) updatedSet.add(ticket.barcode)
            if (ticket.bookingCode.isNotBlank()) updatedSet.add(ticket.bookingCode)
            if (ticket.id > 0) updatedSet.add(ticket.id.toString())
            updatedSet.add("${ticket.movieTitle}_${ticket.dateTime}_${ticket.seats}")
